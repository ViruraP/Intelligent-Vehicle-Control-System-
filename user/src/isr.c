...
void PIT_IRQHandler(void)
{
    if (pit_flag_get(PIT_CH0))
    {
        ...
        // -----------------------------
        // ① 斑马线停车：倒车刹停阶段
        // Car_Stop_Mode: 0=正常 2=倒车刹停 1=最终停车保持
        // -----------------------------
        static uint16 brake_cnt = 0;

        if (Car_Stop_Mode == 2)
        {
            // 倒车刹停：固定反向 PWM（不跑 PID）
            // 下面这两个参数你可以调：
            const uint16 BRAKE_TICKS = 20;    // 倒车刹停时间：25*10ms=250ms（你的 PIT=10ms）
            const uint16 BRAKE_PWM   = 2500;  // 倒车力度：0~10000 之间试（先 2000~3500）

            // 反向
            gpio_set_level(MOTOR1_DIR, MOTOR1_REVERSE_DIR_LEVEL);
            gpio_set_level(MOTOR2_DIR, MOTOR2_REVERSE_DIR_LEVEL);

            pwm_set_duty(MOTOR1_PWM, BRAKE_PWM);
            pwm_set_duty(MOTOR2_PWM, BRAKE_PWM);

            // 计时到 -> 进入停车保持
            brake_cnt++;
            if (brake_cnt >= BRAKE_TICKS)
            {
                brake_cnt = 0;
                Car_Stop_Mode = 1;
            }
        ...
        speed_pwm_l = PidLocCtrl(&speed_pid_l, speed_target - speed_real, 0.01f);
        pwm_set_duty(MOTOR1_PWM, MAX(speed_pwm_l, 0));
        gpio_set_level(MOTOR1_DIR, MOTOR1_FORWARD_DIR_LEVEL);

        speed_real = encoder_data_2;
        speed_pwm_r = PidLocCtrl(&speed_pid_r, speed_target - speed_real, 0.01f);
