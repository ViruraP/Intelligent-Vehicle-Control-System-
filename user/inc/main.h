...
extern int lock;//起始延时
extern uint8_t zebra_mode_cnt ;     // 斑马线模式剩余帧数


//二值化处理阈值
#define FIXED_THRESHOLD 150
...
#define MOTOR1_REVERSE_DIR_LEVEL (GPIO_LOW)
#define MOTOR2_REVERSE_DIR_LEVEL (GPIO_LOW)

// 舵机参数设置
#define SERVO_MOTOR_PWM (PWM4_MODULE2_CHA_C30) // 定义主板上舵机对应引脚
#define SERVO_MOTOR_FREQ (250)                  // 定义主板上舵机频率  请务必注意范围 50-300
#define SERVO_MOTOR_L_MAX (107)  // 定义主板上舵机活动范围(左打方向的极限值) 角度 自行标定  107     107
#define SERVO_MOTOR_R_MAX (79) // 定义主板上舵机活动范围(右打方向的极限值) 角度 自行标定  79     80
